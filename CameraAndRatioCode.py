...
frame = cv2.imread('/home/pi/samplepics/pic7.jpg')
...
def distances():
	...
	for c in cnts:
		...
		for (x, y) in box:
		   ...
		   cv2.putText(image_erode, "{:.1f}in".format(dimA),
					   (int(tltrX - 15), int(tltrY - 10)), cv2.FONT_HERSHEY_SIMPLEX,
					   0.65, (255, 255, 255), 2)
		   cv2.putText(image_erode, "{:.1f}in".format(dimB),
		...
		#object1 = "Object #{i}".format(i)
		#(object+i) = xm #FIX THIS
...
general idea:
	...
	no RHorz or RVert visible -> probably too close so uh go forward
'''		
...
#ratios and movement
#if (4.8 < dimA < 5.2 or 1.8 < dimB < 2.2):
#		right_motor = 0.5
